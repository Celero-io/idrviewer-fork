(function () {
  ...
  IDR.setup = function (config) {
    ...
    for (let i = 1; i <= pgCount; i++) {
      ...
      page.setAttribute(
        "style",
        "width: " + bounds[i - 1][0] + "px; height: " + bounds[i - 1][1] + "px;"
      );
  ...
  const PageManager = (function () {
    ...
    const clearSelection = function (win) {
      try {
        if (win.getSelection) {
          ...
          // IE 11 does not use, maybe older versions do?
    ...
    const handleLoad = function (html, page, callback) {
      ...
      const externalBackground =
        background.getAttribute("data") || background.getAttribute("src");
        ...
        fontFacesString
          ...
          .forEach(function (fontFace) {
            if (fontList.indexOf(fontFace) === -1) {
              ...
              fontsSheet.insertRule(
                fontFace.replace('url("', 'url("' + URL),
                fontsSheet.cssRules.length
              );
            }
          });
      }
  ...
  const LoadManager = (function () {
    ...
    const isLoaded = function (page) {
      return (
        states[page] === PageStates.LOADED || states[page] === PageStates.HIDDEN
      );
    };
    ...
    const unload = function (page) {
      if (
        states[page] === PageStates.LOADED ||
        states[page] === PageStates.HIDDEN
      ) {
    ...
    (function () {
      ...
      Magazine.updateLayout = function () {
        ...
        const viewPortHeight = Math.max(
          pageHeightA,
          pageHeightB,
          mainContainer.clientHeight - paddingY * 2
        );
        pageContainer.style.height = viewPortHeight + "px";
        ...
        for (
          ...
          i <= Math.min(pgCount, currentPage + 2);
          i += 2
        ) {
          // Left page (right when R2L)
          if (i > 0) {
            pages[i].style.marginLeft =
              (isDirectionR2L ? centerX : centerX - getPageWidth(i)) + "px";
            pages[i].style.marginTop =
              centerY - Math.floor(getPageHeight(i) / 2) + "px";
          }
          // Right page (left when R2L)
          if (i < pgCount) {
            pages[i + 1].style.marginLeft =
              (isDirectionR2L ? centerX - getPageWidth(i + 1) : centerX) + "px";
            pages[i + 1].style.marginTop =
              centerY - Math.floor(getPageHeight(i + 1) / 2) + "px";
          }
        }
      };
    ...
    const handleMouseMove = function (e) {
      if (isMouseDown) {
        ...
        mainContainer.scrollLeft =
          mainContainer.scrollLeft + mouseX - e.clientX;
  ...
  const ZoomManager = (function () {
    ...
    const updateZoom = function (value) {
      ...
      lastRulePosition = styleSheet.insertRule(
