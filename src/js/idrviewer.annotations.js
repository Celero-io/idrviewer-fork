(function () {
  ...
  const LoadManager = (function () {
    ...
    const initPage = function (page) {
      const pageContainer = document.getElementById("idrviewer-page" + page);

      const annotationsContainer = document.createElement("div");
      annotationsContainer.className = "idrviewerpage-inner";
      annotationsContainer.setAttribute(
        "style",
    ...
    IDRViewer.on("pageunload", function (data) {
      if (annotationsContainers[data.page]) {
        annotationsContainers[data.page].parentNode.removeChild(
          annotationsContainers[data.page]
        );
    ...
    ActionHandler.ontouchstart = function (data, config) {
      for (let i = 0; i < handlers.touchstart.length; i++) {
        if (data.type === handlers.touchstart[i].type) {
          handlers.touchstart[i].handler.ontouchstart.apply(this, [
            data,
            config,
          ]);
        }
  ...
  const SoundHelper = (function () {
    let currentSound, currentRef;

    return {
      play: function (src, ref) {
        // HTMLAudioElement is not supported in any version of IE
        const isPlaying =
          currentSound && !currentSound.ended && !currentSound.paused;
        if (isPlaying) {
          currentSound.pause();
          if (ref === currentRef) {
            return;
  ...
  (function () {
    ...
    ActionHandler.register(
      ...
      ["click", "mouseover", "mouseout", "setup"],
      LinkActionHandler
    );
  })();
  ...
  (function () {
    const PopupHandler = {};
    let currentPopup, isMobile;

    const createPopup = function (data) {
      if (data["contents"] && data["objref"]) {
        const boundingRect = document
          .querySelector("[data-objref='" + data.objref + "']")
          .getBoundingClientRect();
        const midX =
          (boundingRect.right - boundingRect.left) / 2 + boundingRect.left;
    ...
    PopupHandler.onmouseover = function (data) {
      if (!isMobile) {
        // Prevent mobile/tablet (Android) firing both mouseover and click (shows then immediately hides popup)
        if (!currentPopup) {
  ...
  (function () {
    ...
    const handleSupportedMediaType = function (data, config) {
      const newElement = document.createElement(
        data.action.media.type.substr(0, 5)
      ); // 5 = length of "audio" or "video"
      newElement.setAttribute(
        "style",
      ...
      if (data.action.media.type === "video/mp4") {
        ...
        newElement.setAttribute(
          "src",
          (config.url || "") + data.action.media.src
        );
      }
