(function () {
  ...
  IDRViewer.search = function (
    searchTerm,
    matchCase,
    limitOnePerPage,
    decomposeSnippets = false
  ) {
    if (!textContent) {
    ...
    if (textContent && searchTerm) {
      ...
      for (let i = 0; i < textContent.length; i++) {
        const pageContent = matchCase
          ? textContent[i]
          : textContent[i].toUpperCase();
        let index = -1;
        let resultIndex = 0;
        ...
        do {
          ...
          if (index >= 0) {
            ...
            const snippetStart =
              index >= SNIPPET_LENGTH ? index - SNIPPET_LENGTH : 0;
            const snippetEnd =
              index + searchTerm.length < textContent[i].length - SNIPPET_LENGTH
                ? index + searchTerm.length + SNIPPET_LENGTH
                : textContent[i].length;
            const result = {
              page: i + 1,
              index: resultIndex++,
            };
            if (decomposeSnippets) {
              ...
              result.postSnippet = textContent[i].substring(
                index + searchTerm.length,
                snippetEnd
              );
              result.result = textContent[i].substring(
                index,
                index + searchTerm.length
              );
            } else {
              result.snippet =
                ...
                1 +
                " - " +
                textContent[i].substr(snippetStart, snippetEnd - snippetStart);
            }
            results.push(result);
          }
        } while (
          !limitOnePerPage &&
          index !== -1 &&
          (!resultsLimit || results.length < resultsLimit)
        );
      }
    }
  ...
  IDRViewer.prevSearchResult = function () {
    const result =
      selectedResult >= 0
        ? results[
            selectedResult - 1 >= 0 ? selectedResult - 1 : results.length - 1
          ]
        : results.toReversed().find((result) => result.page <= curPg) ||
  ...
  IDRViewer.selectSearchResult = function (page, index) {
    ...
    const resultIndex = results.findIndex(
      (result) => result.page === page && result.index === index
    );
...
(function () {
  ...
  IDRViewer.on("searchready", () => {
    IDRViewer.on("pageload", function (page) {
      ...
      findSearchTermsInPage(
        document.querySelector("#idrviewer-page" + page.page)
      );
      if (selectedMatch && selectedMatch.page === page.page) {
        matches[page.page].pageHandler.setMatchDescriptorSelected(
          selectedMatch.index,
          true
        );
      }
  ...
  function findSearchTermsInPage(page) {
    ...
    if (
      ...
      !matches.hasOwnProperty(getPageNumberFromId(page.id))
    ) {
      return;
    }
    ...
    if (
      pageHandler === SVGPageHandler &&
      textElements[0].getComputedTextLength() === 0
    ) {
    ...
    for (const textElement of textElements) {
      const mappings =
        textElement.dataset.mappings !== undefined
          ? JSON.parse(textElement.dataset.mappings)
          : undefined;
      ...
      if (mappings) {
        ...
        const specials = [...rawText.matchAll(/&(?:gt|lt|amp);/gi)].reduce(
          (acc, match) => {
            acc.push({
              index:
                match.index -
                acc.reduce((acc2, value) => acc2 + value.offset, 0),
        ...
        for (const mapping of mappings) {
          ...
          const index =
            mapping[0] +
            differences.reduce((acc, value) => acc + value.offset, 0) +
            specials
              .filter((value) => value.index < mapping[0])
      ...
      while (i < text.length) {
        for (
          elementOffset = 0;
          i + elementOffset < text.length &&
          searchOffset < search.searchTerm.length;
          ...
          else if (insideSpecial && text.charAt(i + elementOffset) === ";")
            insideSpecial = false;

          if (
            text.charAt(i + elementOffset) ===
            search.searchTerm.charAt(searchOffset)
          ) {
            elementOffset++;
            searchOffset++;
          } else {
        ...
        if (elementOffset > 0) {
          ...
          if (!matchDescriber.elements.length) {
            ...
            for (const difference of differences) {
              ...
              matchDescriber.beginning -=
                matchDescriber.beginning < difference.index + difference.offset
          ...
          if (searchOffset === search.searchTerm.length) {
            ...
            for (const difference of differences) {
              ...
              matchDescriber.end -=
                matchDescriber.end < difference.index + difference.offset
  ...
  function clearSelectedMatch() {
    if (Object.keys(matches) && selectedMatch) {
      ...
      match?.pageHandler?.setMatchDescriptorSelected(
        selectedMatch.index,
        false
      );
    }
  ...
  let SVGPageHandler = function (page) {
      ...
      if (matchDescriber.elements.length === 1) {
        ...
        highlightBounds.push(
          getHighlightBounds(
            startExtent.x,
            startExtent.y,
            endExtent.x + endExtent.width - startExtent.x,
            endExtent.y + endExtent.height - startExtent.y,
    ...
    svgPageHandler.storeDescriber = function (matchDescriber) {
      svgPageHandler.searchResults = svgPageHandler.searchResults.concat(
        calculateHighlights(matchDescriber)
      );
    };
    ...
    svgPageHandler.highlightResults = function () {
      for (let i = 0; i < svgPageHandler.searchResults.length; i++) {
        ...
        bounds.refElement.parentNode.insertBefore(
          createHighlightFromBounds(bounds, i),
          bounds.refElement
        );
      }
    };
    ...
    svgPageHandler.setMatchDescriptorSelected = function (index, selected) {
      const highlights = page
        ...
        .contentDocument.querySelectorAll(
          '[data-index="' + makeIndexString(index) + '"].highlight'
        );
  ...
  let HTMLPageHandler = function (page) {
    ...
    function updateChangedElements(changedElements, element) {
      ...
      const length = 51;
      const changedElement = changedElements.find(
      ...
      for (let i = 0; i < htmlPageHandler.searchResults.length; i++) {
        ...
        if (describer.elements.length === 1) {
          ...
          element.innerHTML = text.substring(
            0,
            describer.beginning + elementOffset
          );

          let highlightString = makeHighlightString(
            text.substring(
              describer.beginning + elementOffset,
              describer.end + elementOffset
            ),
            i
          );
          ...
          let highlightString = makeHighlightString(
            text.substring(describer.beginning + elementOffset, text.length),
            i
          );
          element.innerHTML += highlightString;
          ...
          element.innerHTML = "";
          highlightString = makeHighlightString(
            text.substring(0, describer.end),
            i
          );
          element.innerHTML += highlightString;
    ...
    htmlPageHandler.setMatchDescriptorSelected = function (index, selected) {
      const highlights = page.querySelectorAll(
        '[data-index="' + makeIndexString(index) + '"].highlight'
      );
    ...
    htmlPageHandler.scrollIntoView = function (index) {
      const highlightedElement =
        htmlPageHandler.searchResults[index].elements[0];
